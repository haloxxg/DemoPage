...
/*模拟下拉菜单*/
jQuery.EasyUIselect = function(divselectid,inputselectid) {
...
/*折叠*/
jQuery.EasyUIfold = function(obj,obj_c,speed,obj_type,Event){
	if(obj_type == 2){
...
/*返回顶部*/
var $backToTopEle=$('<a href="javascript:void(0)" class="EasyUI-iconfont toTop" title="返回顶部" alt="返回顶部" style="display:none">&#xe684;</a>').appendTo($("body")).click(function(){
...
/*EasyUImodalalert*/
function EasyUImodal_alert(info,speed){
	if(speed==0||typeof(speed) == "undefined"){
		...
		$("#modal-alert").fadeIn();
		setTimeout("EasyUImodal_alert_hide()",speed);
	}	
}
function EasyUImodal_alert_hide() {
...
$(function(){
	/*****表单*****/
    $.EasyUIfocusblur(".input-text,.textarea");
	/*按钮loading*/
	...
	/**/
	$.EasyUIselect("#divselect","#inputselect");
	...
	/*EasyUIalert*/
	$.EasyUIhover('.EasyUIalert i');
	$(".EasyUIalert i").on("click",function(){
		var EasyUIalert = $(this).parents(".EasyUIalert");
		EasyUIalert.fadeOut("normal",function(){
		  EasyUIalert.remove();
		});
	});
	...
	/*tag标签*/
	var time1;
	$(".main-tags-lable").show();
	$(".main-tags-input").val("");
	$(document).on("blur",".main-tags-input",function(){
		time1 = setTimeout(function(){
			$(this).parents(".main-tags").find(".main-tags-list").slideUp();
		}, 400);
	});
	$(document).on("focus",".main-tags-input",function(){
		clearTimeout(time1);
	});
	$(document).on("click",".main-tags-input",function(){
		$(this).find(".main-tags-input").focus();
		$(this).find(".main-tags-list").slideDown();
	});
	function gettagval(obj){
		var str ="";
		var token =$(obj).parents(".main-tags").find(".main-tags-token");
		//alert(token.length)
		if(token.length<1){
			$(obj).parents(".main-tags").find(".main-tags-val").val("");
			return false;
		}
		for(var i = 0;i< token.length;i++){
			str += token.eq(i).text() + ",";
			$(obj).parents(".main-tags").find(".main-tags-val").val(str);
		}
	}
	$(document).on("keydown",".main-tags-input",function(event){
		$(this).next().hide();
		var v = $(this).val().replace(/\s+/g, "");
		var reg=/^,|,$/gi;
		v=v.replace(reg,"");
		v=$.trim(v);
		var token =$(this).parents(".main-tags").find(".main-tags-token");
		if(v!=''){
			if(event.keyCode==13||event.keyCode==108||event.keyCode==32){
				$('<span class="EasyUI-tags-token">'+v+'</span>').insertBefore($(this).parents(".main-tags").find(".main-tags-iptwrap"));
				$(this).val("");
				gettagval(this);
			}
		}else{
			if(event.keyCode==8){
				if(token.length>=1){
					$(this).parents(".main-tags").find(".main-tags-token:last").remove();
					gettagval(this);
				}
				else{
					$(this).parents(".main-tags").find(".main-tags-lable").show();
					return false;
				}
	...
	$(document).on("click",".main-tags-has span",function(){
		var taghasV = $(this).text();
		taghasV=taghasV.replace(/(^\s*)|(\s*$)/g,"");
		$('<span class="EasyUI-tags-token">'+taghasV+'</span>').insertBefore($(this).parents(".main-tags").find(".main-tags-iptwrap"));
		gettagval(this);
		$(this).parents(".main-tags").find(".main-tags-input").focus();
	});
	$(document).on("click",".main-tags-token",function(){
		var token =$(this).parents(".main-tags").find(".main-tags-token");
		var it = $(this).parents(".main-tags");
		$(this).remove();
		switch(token.length){
			case 1 : it.find(".main-tags-lable").show();
			break;
		}
		var str ="";
		var token =it.find(".main-tags-token");
		//alert(token.length)
		if(token.length<1){
			it.find(".main-tags-val").val("");
			return false;
		}
		for(var i = 0;i< token.length;i++){
			str += token.eq(i).text() + ",";
			it.find(".main-tags-val").val(str);
		}		
	});
});
